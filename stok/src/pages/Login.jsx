...
/**
 * Login.jsx (stabil, no-console-spam)
 * - Prioritaskan /config.json (public) agar tidak bergantung backend
 * - Fallback ke /api/config, /config, :3001, :3000
 * - Kalau semua gagal: pakai default aman + tampilkan peringatan halus
 */
...
export default function Login() {
  ...
  // ================ helper: fetch dengan timeout & silent 404 ================
  const fetchJsonSilent = async (url, { timeoutMs = 3500 } = {}) => {
    ...
    try {
      ...
      if (!res.ok) {
        // 404 dianggap "tidak ada" dan tidak menimbulkan noise
        if (res.status !== 404) {
          console.debug(`[config] ${url} -> HTTP ${res.status} ${res.statusText}`);
        }
        return null;
      }
      return await res.json().catch(() => null);
    } catch (e) {
      // Connection refused / CORS / timeout → tenangkan log
      console.debug(`[config] ${url} gagal: ${e?.name || "Error"} ${e?.message || e}`);
      return null;
    } finally {
      clearTimeout(t);
    }
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      ...
      // Urutan prioritas: public → api → config → local dev ports
      const endpoints = [
      ...
      for (const u of endpoints) {
        console.debug("[config] mencoba:", u);
        const json = await fetchJsonSilent(u);
        if (json && typeof json === "object") {
          found = json;
          console.info("[config] OK dari:", u);
          break;
        }
      }
      ...
      if (normalized) {
        ...
        console.warn("[config] Semua endpoint gagal, memakai fallback bawaan.");
      }
    ...
    // sengaja dependency kosong → hanya jalan sekali saat mount
  }, []);
  ...
  useEffect(() => {
    ...
    console.log(`[route] email=${currentEmail} admin=${isAdmin}`);
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      // Coba silent login dulu kalau sudah ada session
      try {
        await instance.ssoSilent({});
      } catch {
        // jika gagal, lanjut popup
